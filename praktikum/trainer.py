...
print(debt_purpose[debt_purpose['income_type'] == 'пенсионер'].groupby('purpose')['total_income'].count())
print(debt_purpose[debt_purpose['income_type'] == 'пенсионер'].groupby('dob_years')['total_income'].count())
#
...
# def add_child_category(row: str):
#     if row == 0:
#         return False
#     return True
#
...
# children['have_child'] = client_data['children'].apply(add_child_category)
# children = children.pivot_table(index='have_child', columns='debt', values='total_income', aggfunc='count', margins=True, margins_name='total_loans')
# children.rename(columns={0: 'good_loans', 1: 'bad_loans'}, inplace=True)
# children['fail_cr%'] = children['bad_loans'] / children['total_loans']
# children.style.format({'fail_cr%': '{:.2%}'})
# print(children)
