import requests as r
import pandas as pd
import datetime

API_URL = 'https://api-lime-finance.affise.com/'
API_KEY = '0a3994e5f04ed3d755cba60eb50de7c6'

date_from = '2021-03-01'
date_to = datetime.datetime.now()
status = 1
limit = 2000

resp = r.get(API_URL + '3.0/stats/conversions', headers={'API-Key': API_KEY},
             params=(
                 ('date_from', date_from),
                 ('date_to', date_to),
                 ('status', status),
                 ('limit', limit))
             ).json()

pages = resp['pagination']['total_count'] // resp['pagination']['per_page'] + 1

conversion_list = []
for page in range(pages):
    resp = r.get(API_URL + '3.0/stats/conversions', headers={'API-Key': API_KEY},
                 params=(
                     ('date_from', date_from),
                     ('date_to', date_to),
                     ('status', status),
                     ('limit', limit),
                     ('page', page + 1)
                 )).json()
    for conversions in resp['conversions']:
        conversion_list.append(conversions)

data = []
columns = ['ts', 'project', 'offer_title', 'goal_name', 'goal_id', 'payouts',
           'sub1', 'sub2', 'sub3', 'sub4', 'sub5', 'sub6', 'affiliate']
for conversion in conversion_list:
    ts = conversion['created_at']
    project = conversion['advertiser']['title']
    offer_title = conversion['offer']['title']
    goal_name = conversion['goal']
    goal_id = conversion['action_id']
    payouts = conversion['payouts']
    sub1 = conversion['sub1']
    sub2 = conversion['sub2']
    sub3 = conversion['sub3']
    sub4 = conversion['sub4']
    sub5 = conversion['sub5']
    sub6 = conversion['sub6']
    affiliate = conversion['partner']['title']

    data.append([ts, project, offer_title, goal_name, goal_id, payouts, sub1, sub2, sub3, sub4, sub5, sub6, affiliate])

data_frame = pd.DataFrame(data=data, columns=columns)
data_frame['ts'] = data_frame['ts'].astype('datetime64[D]')
data_frame['week'] = data_frame['ts'].dt.isocalendar().week
data_frame['month'] = data_frame['ts'].dt.month

lime = data_frame.query('project == "Lime" and affiliate == "Guruleads"')

lime_pivot = lime.pivot_table(index='week', columns='sub3', values='project',
                              aggfunc='count', margins=True).fillna(0).sort_values(by='All', axis=1, ascending=False).astype('int')
lime_pivot.to_csv('guru.csv')
