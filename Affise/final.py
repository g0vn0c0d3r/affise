...
def get_conversions_and_cost_by_webmaster(*, offer_id: int, partner_id: int, date_from: str, date_to: str):
    ...
    conversions = final_data_frame.groupby(by='sub3')['goal_name'].count()
    budget = final_data_frame.groupby(by='sub3')['goal_value'].sum()
    fin = pd.concat([conversions, budget], axis=1)
